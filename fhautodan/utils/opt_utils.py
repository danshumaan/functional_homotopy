...
def replace_with_synonyms(sentence, num=10):
    T = {"llama-2", "llama-3", "llama2", "meta", "vicuna_v1.1", "lmsys", "guanaco", "theblokeai", "wizardlm", "mpt-chat",
...
def get_score_autodan(tokenizer, conv_template, instruction, 
                      target, model, device, template_name, 
                      test_controls=None, crit=None, dis=None):
...
def get_score_autodan_med_memory(tokenizer, conv_template, instruction, 
                                 target, model, device, template_name,
                                 batch_split=1,
                                 test_controls=None, crit=None, dis=None):
    # Convert all test_controls to token ids and find the max length
    ...
    input_ids_tensor = torch.stack(padded_input_ids_list, dim=0)
    tt = []
    for k in range(len(temp_logits)):
        for logit in temp_logits[k]:
            tt.append(logit)
    ...
    for idx, target_slice in enumerate(target_slices):
        loss_slice = slice(target_slice.start - 1, target_slice.stop - 1)
        logits_slice = temp_logits[idx][loss_slice, :].unsqueeze(0).transpose(1, 2)
        targets = input_ids_tensor[idx, target_slice].unsqueeze(0)
        if dis is not None:
            logits_cal = logits_slice.unsqueeze(0).expand_as(dis)
            loss = -crit(logits_cal, dis).mean()
...
def get_score_autodan_low_memory(tokenizer, conv_template, instruction, 
                                 target, model, device, 
                                 batch_split,
                                 template_name, test_controls=None,
                                 crit=None):
    losses = []
    for item in test_controls:
        suffix_manager = autodan_SuffixManager(tokenizer=tokenizer,
